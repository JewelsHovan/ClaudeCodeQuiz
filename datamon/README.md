...
- `battle-arena.js` — strict five-domain authored arena manifest, one-resident lazy loader, timeout, and fallback
...
- `battle-arenas/` — five reviewed true-detail 1600×864 classic domain theaters plus strict manifest
...
Classic battles use five background-only certification theaters generated by the pinned OpenRouter
route `openai/gpt-5.4-image-2`. The current source brief forbids people, creatures, portraits,
text, logos, HUD, and stateful values; roster and Battlemon identities remain separate packaged
sprites. The accepted MCP pilot retains its exact historical v1 prompt provenance, which allowed
only unidentifiable distant silhouettes. Provider responses, generated images, style reference,
and the append-only authorization ledger stay in ignored `datamon/.battle-arena-ai-raw/`.
Deterministic Pillow processing crops the reviewed camera, resizes to true-detail 1600×864,
limits each background to 256 colors, and validates contrast, measured positive costs, cumulative
spend, exact files, and hashes. Candidate processing never mutates the accepted batch; acceptance
binds both review sheets before one rollback-safe atomic promotion. Every planned billable attempt is
persisted with its reservation before dispatch; concurrent results are fully drained and archived.
An ambiguous transport/cost result keeps its reservation in cumulative spend and blocks generation
until a human reconciles the provider account.

```bash
# Explicit billable path. style-reference.png must already exist in ignored raw storage.
OPENROUTER_API_KEY=... python3 datamon/tools/gen_battle_arena_ai.py \
  --generate --missing --max-spend 2 --concurrency 2
# Once an accepted batch exists, any newly approved billable candidate must also pass
# --new-candidate; preflight reserves every planned call against the $50 authorization.

# Offline candidate processing in ignored raw storage; the accepted runtime stays untouched.
# Inspect both color and grayscale t55-domain-arenas sheets.
python3 datamon/tools/gen_battle_arena_ai.py --pipeline-only
python3 datamon/tools/gen_battle_arena_ai.py --accept <reviewed-contact-sheet-sha256>

# With ignored raw evidence available, rebuild twice and prove the accepted batch was untouched.
python3 datamon/tools/gen_battle_arena_ai.py --validate-twice --validate
# A clean checkout can always validate the tracked accepted contract without private raw evidence.
python3 datamon/tools/gen_battle_arena_ai.py --validate
```

The title fetches only `battle-arenas/manifest.json`. A classic encounter requests its one active
domain image alongside the encounter's 1–3 Battlemon sheets. Requests coalesce, manifest/image
stalls converge after five seconds, only one decoded 5.3MiB arena remains resident, and malformed
metadata cannot authorize another path. Transient Battlemon fallback sheets are released as soon as
accepted sheets decode; exact arena, arena-fallback, Battlemon, and Battlemon-fallback bytes keep the
normal classic presentation at ≤8MiB. The deterministic command-deck fallback keeps combat playable. Packaging independently pins the model, color/grayscale review receipts, positive
per-image measured costs, `$6.811188 / $50` authorization-wide spend, aggregate digest, five RGB
PNGs, and exclusion of raw/review material. Rollback removes `battle-arena.js`,
`battle-arenas/`, and their declarations; no save migration is involved.
